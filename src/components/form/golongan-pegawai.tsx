...
export const formSchema = z4.object({
  ...
  golongan_i: z4.coerce.number<number>().min(0),
  golongan_ii: z4.coerce.number<number>().min(0),
  golongan_iii: z4.coerce.number<number>().min(0),
  golongan_iv: z4.coerce.number<number>().min(0),
});

type FormData = z4.infer<typeof formSchema>;

interface FormGolonganPegawaiProps {
  initialData?: FormData; // pass this if editing
  onSuccess?: () => void; // callback after successful submission
}

export function FormGolonganPegawai({
  initialData,
  onSuccess,
}: FormGolonganPegawaiProps) {
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormData) => {
      ...
      const url = data.id
        ? `/api/golongan-pegawai/${data.id}`
        : "/api/golongan-pegawai";
    ...
    <Form {...form}>
      ...
      >
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="col-span-full">
              <FormLabel>Golongan I</FormLabel>
              <FormControl>
                <Input
                  type="number"
                  placeholder="0"
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="col-span-full">
              <FormLabel>Golongan II</FormLabel>
              <FormControl>
                <Input
                  type="number"
                  placeholder="0"
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="col-span-full">
              <FormLabel>Golongan III</FormLabel>
              <FormControl>
                <Input
                  type="number"
                  placeholder="0"
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="col-span-full">
              <FormLabel>Golongan IV</FormLabel>
              <FormControl>
                <Input
                  type="number"
                  placeholder="0"
